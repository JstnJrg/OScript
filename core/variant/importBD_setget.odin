...
import_get   :: #force_inline proc( id : ImportID, property,variant_return: ^Value, has_error: ^bool, call_error_p: ^ObjectCallError) {
	...
	@static cannot := ValueBitSet{.OBJ_PACKAGE} //Nota(jstn): restrição de objectos que não pode acessar
	cannot_assign  := false
...
// // Nota(jstn): para pacotes nativos
// import_get_bultin   :: #force_inline proc( id : ImportID, property,variant_return: ^Value, has_error: ^bool, call_error_p: ^ObjectCallError) {
